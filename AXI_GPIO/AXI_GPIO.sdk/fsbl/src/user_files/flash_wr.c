...
int QspiFlashPolledExample(XQspiPs *QspiInstancePtr, u16 QspiDeviceId)
{
	...
	//使用普通读取命令读出数据
	FlashRead1(QspiInstancePtr, TEST_ADDRESS, MAX_DATA, READ_CMD);
	...
	//使用快速读取命令读出数据
	memset(ReadBuffer, 0x00, sizeof(ReadBuffer));
	FlashRead1(QspiInstancePtr, TEST_ADDRESS, MAX_DATA, FAST_READ_CMD);
	...
	//使用双重读取命令读取数据
	memset(ReadBuffer, 0x00, sizeof(ReadBuffer));
	FlashRead1(QspiInstancePtr, TEST_ADDRESS, MAX_DATA, DUAL_READ_CMD);
	BufferPtr = &ReadBuffer[DATA_OFFSET + DUMMY_SIZE];
	...
	//使用块读取命令读取数据
	memset(ReadBuffer, 0x00, sizeof(ReadBuffer));
	FlashRead1(QspiInstancePtr, TEST_ADDRESS, MAX_DATA, QUAD_READ_CMD);
...
void FlashRead1(XQspiPs *QspiPtr, u32 Address, u32 ByteCount, u8 Command)
{
	WriteBuffer[COMMAND_OFFSET]   = Command;
	WriteBuffer[ADDRESS_1_OFFSET] = (u8)((Address & 0xFF0000) >> 16);
	WriteBuffer[ADDRESS_2_OFFSET] = (u8)((Address & 0xFF00) >> 8);
